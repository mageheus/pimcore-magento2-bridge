...
class ObjectListener
{
     ...
     */
    public function __construct(protected IntegrationConfigurationService $integrationService, protected EventDispatcherInterface $eventDispatcher, protected IntegratedObjectEventFactory $eventObjectFactory)
    {
    }
    ...
    protected function getIntegrationsToDelete(DataObject\Concrete $object): array
    {
        ...
        if ($originObject && $object->isPublished()) {
            ...
            $diff                     = array_udiff(
                ...
                $objectIntegrations,
                fn($integration1, $integration2) => $integration1->getId() - $integration2->getId()
            );
            return $diff;
