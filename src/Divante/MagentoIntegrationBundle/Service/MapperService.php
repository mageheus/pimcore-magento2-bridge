...
class MapperService
{
    public const ELEMENTS_PROPERTY_NAME = 'elements';

    /**
     * MapperService constructor.
     * @param MapperContext $mapper
     */
    public function __construct(protected MapperContext $mapper)
    {
    }
    ...
    {
        $object = new \stdClass();
        $mappingArray = match ($type) {
            MapperHelper::OBJECT_TYPE_PRODUCT => $configuration->getDecodedProductMapping(),
            MapperHelper::OBJECT_TYPE_CATEGORY => $configuration->getDecodedCategoryMapping(),
            default => throw new InvalidArgumentException("Element type not known"),
        };
        ...
        foreach ($out as $key => $value) {
            if ($key != self::ELEMENTS_PROPERTY_NAME) {
                ...
                $value = array_filter($value, fn($elem) => $this->canAttributeBeProcessed($elem, $objectClass, $configuration));
                /** @var Element $element */
                foreach ($value as $element) {
                    $label = $objectClass->getFieldDefinition($element->name)->getTitle();
                    $element->label = $label ?: $element->name;
                    $this->mapper->map(
                        $element,
                        $elements,
                        $mappingArray,
    ...
    protected function getFieldConfiguration(DataObject\ClassDefinition\Data $field): FromColumn
    {
        ...
        $fromColumn->setLabel($field->getTitle() ?: $field->getName());
    ...
    public function getAttributesChecksum(\stdClass $object): array
    {
        ...
        ksort($attributes);
        return ['algo' => 'md5', 'value' => md5(json_encode($attributes, JSON_THROW_ON_ERROR))];
    }
        ...
        foreach ($brick->value as $singleField) {
            ...
            if ($definition instanceof DataObject\ClassDefinition\Data\Multiselect) {
                ...
                foreach ($singleField->value as $value) {
                    $values = array_filter($options, fn($elem) => $elem['value'] == $value);
                }
