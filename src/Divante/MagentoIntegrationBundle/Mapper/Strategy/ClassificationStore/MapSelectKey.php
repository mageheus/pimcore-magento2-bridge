...
{
    public const ALLOWED_TYPES_ARRAY = ['select', 'multiselect'];
    public const SINGLE_TYPE = 'select';
    public const MULTI_TYPE = 'multiselect';
    ...
    public function map(
        ...
        if (count((array) $valuesWithKey) == 1) {
    ...
    {
        $selectDefinition = json_decode($field->getDefinition(), null, 512, JSON_THROW_ON_ERROR);
        return array_map(
            fn($elem) => ['key' => $elem->key, 'value' => $elem->value],
